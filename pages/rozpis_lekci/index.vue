<template>
  <v-container>
    <v-layout class="mt-5" justify-center row v-bind="binding" ref="app">
      ...
      <v-flex xs12 sm6>
        <div class="ml-4">
          ...
          <p>Rezervace na stránkách <a href="www.tjviktoriavestec.cz">www.tjviktoriavestec.cz</a></p>
          <p>   v sekci "cvičení-Zuzi"</p>
          <br><br>
          <p>mob. 777 100 015</p>
          <p>email: <a href="mailto:cvicovestec@gmail.com">cvicovestec@gmail.com</a></p>
          <br><br>
    ...
    <v-layout column class="mt-5">
      ...
      >
        <v-divider></v-divider>
        <h2 class="my-4 accent--text exercise-title">{{ item.title }}</h2>
        <p class="mb-4 accent--text exercise">{{ item.info }}</p>
      </v-flex>
    </v-layout>
  </v-container>
</template>
  ...
  data () {
    return {
      items: [
        ...
        { title: 'Bosu cardio/body', info: 'Při lekci využijeme speciální balanční pomůcku bosu balance trainer, díky které je cvičení mnohem efektivnější - kromě běžných svalů zapojíme i vnitřní stabilizační systém (hluboké svaly). Cvičení je zaměřené na formování postavy, rozvoje síly, stability a zlepšení fyzické kondice.' },
        { title: 'Bosu slow', info: 'Jedná se o lekci body&mind, kdy v pomalém tempu zaposilujeme a zároveň protáhneme svaly celého těla, včetně našeho core (středu těla). Cvičení probíhá naboso s využitím balanční pomůcky bosu balance trainer.' },
        { title: 'Power jóga', info: 'Vychází z klasické jógy, avšak jednotlivé pozice se dynamicky střídají. Je zaměřená na posílení středu těla a práci hlubokého stabilizačního systému. Cvičení vede k posílení a protažení svalů celého těla, rozvíjí koordinaci, sílu, flexibilitu, zároveň se však naučíte správně a hluboce dýchat.' },
        { title: 'Pilates', info: 'Je cvičební systém, který je zaměřen na posílení svalů celého těla, zejména však hlubokých břišních a zádových svalů a svalů pánevního dna. Rozvíjí sílu, ohebnost, koordinaci a dýchání. Pravidelné cvičení pomáhá od bolesti zad a zlepšuje držení celého těla.' },
        { title: 'Kruhový trénink', info: 'V průběhu lekce vystřídáte v pravidelných intervalech několik stanovišť, z nichž na každém jsou připraveny cviky na různé svalové skupiny (někdy s pomůckou, někdy jen s vlastní váhou). Efektivně tak procvičíte celé tělo, formujete postavu, rozvíjíte sílu a vytrvalost. Cvičení je vhodné pro muže i ženy.' },
        ...
        { title: 'Bodystyling', info: 'Po krátkém zahřátí následuje posilování svalů celého těla. Občas i s využitím cvičebních pomůcek jako bosu balance trainer, činek, gumiček, převážně však cvičíme jen s vlastní váhou těla. Lekce je zaměřená na rozvoj síly a formováí postavy.' },
        { title: 'Dance aerobic', info: 'Aerobní cvičení, ve kterém se postupně naučíme lehkou sestavu s využitím tanečních prvků. Perfektní pro odreagování, spálení kalorií a tvarování postavy zábavnou formou.' },
        { title: 'Fitbox', info: '​Intenzivní aerobní cvičební s využitím speciálních totemů. Prvky boxu jsou přizpůsobeny běžné klientele, zvládne skutečně každý. Při cvičení se nejen zapotíte, ale především odbouráte veškerý stres. Zároveň posilujete a formujete postavu. Vhodné pro muže i ženy, začátečníky i pokročilé.' }
      ]
    }
  },
  computed: {
